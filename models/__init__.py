...
class DataSet:
    ...
    def __post_init__(self):
        self.data = np.array(self.data, dtype=object if isinstance(self.data, list) and len(self.data) > 0 and isinstance(self.data[0], list) else float)
        if self.data.ndim == 1:
            ...
            if self.column_names is None:
                self.column_names = [f"col_{i}" for i in range(self.n_cols)]
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "missing_count": int(np.sum(np.isnan(self.data.astype(float, errors="ignore")) if self.data.dtype != object else [self._is_missing(v) for v in self.data.flat]))
        }
    ...
    def get_clean_column(self, col_idx: int = 0) -> np.ndarray:
        if self._is_matrix:
            col = self.data[:, col_idx].astype(float, errors="ignore")
        else:
            col = self.data.astype(float, errors="ignore")
    ...
    def get_nan_count(self, col_idx: Optional[int] = None) -> int:
        if col_idx is not None:
            clean = self.get_clean_column(col_idx)
            total = self.n_rows
            return total - len(clean)
        else:
            return int(np.sum([self.get_nan_count(i) for i in range(self.n_cols)]))
